...
namespace elasto_cuda
{
	...
	void SaveDispDataFileFilter(const char *filename,   cv::Mat &  pmat);
        ...
        if (idx < in.cols && idy < in.rows)
        {
			...
			if (idx <= params.cols - 1)       {            //small 
				...
				for (i = 0; i <= idx; i++)
				{
					...
					sum += pParams[i] * pIn[in.cols-1-idx+i];
	...
	void CudaComputer::DoFilter_1(cv::Mat &in_out, PtrStepSzf params)
	{
		...
		//save  the result   change   by   wong  


		SaveDispDataFileFilter("bpresult.dat", in_out);
	...
	void  SaveDispDataFileFilter(const char *filename, cv::Mat & pmat)
	{
		...
		if (err == 0)
		{
			...
			{
				float* data = pmat.ptr<float>(i);

				for (j = 0; j < pmat.cols; j++)
				{

					fprintf_s(file, "%f ", data[j]);
