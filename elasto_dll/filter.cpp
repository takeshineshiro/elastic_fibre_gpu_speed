...
extern SMModeInput s_MModeParam;
...
//////////////////////////////////////////////////////////////////////////
// 先做对数压缩，然后制作成bitmap文件。
// 不理解为何要做一次19阶的滤波。出于减少处理时间的目的，我把它去掉了。20160113
//
//////////////////////////////////////////////////////////////////////////
void Filter::grayImage(const std::string &filename, CvMat *pmat, int db)
{
	...
	for (int i = 0; i < pmat->cols; i++)
	{
		for (int j = 0; j < pmat->rows; j++)
		{
			if (CV_MAT_ELEM(*pmat, float, j, i) > max) max = CV_MAT_ELEM(*pmat, float, j, i);
		}
	}	
	...
	for (int i = 0; i < pmat->cols; i++)
	{
		...
		{
			temp = static_cast<float*>(static_cast<void*>(image->imageData + j * image->widthStep + i * sizeof(float)));
			CV_MAT_ELEM(*pmat, float, j, i) /= max;
			if (CV_MAT_ELEM(*pmat, float, j, i) < x) CV_MAT_ELEM(*pmat, float, j, i) = x;
			CV_MAT_ELEM(*pmat, float, j, i) = 20 * log10(CV_MAT_ELEM(*pmat, float, j, i));
			CV_MAT_ELEM(*pmat, float, j, i) = (CV_MAT_ELEM(*pmat, float, j, i) + db) / db;
			*temp = 255 * CV_MAT_ELEM(*pmat, float, j, i);
		}
	}	
...
void Filter::hilbert(const std::string &filename, const float *data , float *filterdata, int dn)
{
	...
	if (!filename.empty())
	{
		...
		CvSize image_sz = cvSize(s_MModeParam.nEMatW, s_MModeParam.nEMatH); 

		IplImage *image = cvCreateImage(image_sz, IPL_DEPTH_32F, 1);

		//cvNamedWindow("Win",0);
		//cvResizeWindow("Win",300,300);
		int  step = (int)ceil((double)dn / s_MModeParam.nEMatH);

		CvPoint pt_now, pt_pre;
		cvZero(image);

		int  w = s_MModeParam.nEMatW - 5;
		for (int i = 0; i < dn; i++)
		{
			filterdata[i] = filterdata[i] / max;
			pt_now.x = (int)(w * filterdata[i]) + 5;
			pt_now.y = (int)(i/step);
